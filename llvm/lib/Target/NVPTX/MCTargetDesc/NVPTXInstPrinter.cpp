...
void NVPTXInstPrinter::printRegName(raw_ostream &OS, MCRegister Reg) {
  ...
  if (Kind == NVPTX::VirtualRegisterKind::Dice) {
    const unsigned Num = Reg.id() & NVPTX::VirtualRegisterNumMask;
    OS << '%' << NVPTX::DiceClassChars[Num >> NVPTX::DiceClassShift]
       << (Num & ~NVPTX::DiceClassMask);
    return;
  }
